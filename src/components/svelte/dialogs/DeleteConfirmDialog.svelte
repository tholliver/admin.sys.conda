<script lang="ts">
    ...
    const labels: Record<
        EntityType,
        { title: string; desc: string; action: string; color: string }
    > = {
        employee: {
            title: "Dar de baja al miembro del personal",
            desc: "El miembro del personal quedará inactivo y no podrá recibir salarios. Esta acción puede revertirse editando su estado.",
            action: "Dar de baja",
    ...
    async function handleDelete() {
        ...
        try {
            ...
            if (result?.error) {
                serverError =
                    result.error.message ?? "Error al procesar la acción.";
                return;
            }
...
>
    ...
    {:else}
        <div class="flex flex-col gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">
                        {entityName}{entityLabel ? ` — ${entityLabel}` : ""}
                    </p>
                </div>
            </div>
            ...
            {#if serverError}
                <div
                    class="flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2"
                >
