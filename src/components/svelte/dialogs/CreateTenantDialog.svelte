<script lang="ts">
  ...
  function reset() {
    name = ci = ciCity = phone = email = roomNumber = floor = description = monthlyRent = startDate = endDate = notes = "";
    ciCity = "CB";
    inputErrors = {};
    serverError = null;
    successMsg = null;
...
<Dialog bind:isOpen size="lg" title="Registrar Inquilino" description="Complete los datos del nuevo cliente" onClose={reset}>
  ...
  {:else}
    ...
    <form onsubmit={handleSubmit} class="space-y-4">
      ...
      <div>
        ...
        />
        {#if fieldError('name')}
          <p class="mt-1 text-xs text-red-600 flex items-center gap-1"><CircleAlert class="h-3 w-3" />{fieldError('name')}</p>
        {/if}
      </div>
      ...
      <div class="grid grid-cols-3 gap-3">
        ...
        <div>
          ...
          {#if fieldError('startDate')}
            <p class="mt-1 text-xs text-red-600">{fieldError('startDate')}</p>
          {/if}
        </div>
        <div>
      ...
      <div class="flex items-center justify-between border-t border-slate-100 pt-4">
        ...
        <button
          type="submit"
          disabled={isSubmitting || !name || !roomNumber || !monthlyRent || !startDate}
