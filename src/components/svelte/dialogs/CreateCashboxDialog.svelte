  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    try {
      ...
      if (result?.data?.success) {
        successMsg = result.data.message ?? "Caja creada correctamente.";
        setTimeout(() => {
          isOpen = false;
          reset();
...
<Dialog bind:isOpen size="md" title="Nueva Caja" description="Crea una cuenta o caja para gestionar fondos" onClose={reset}>
    ...
    <form onsubmit={handleSubmit} class="space-y-4">
      ...
      <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">
          ...
          <span class="text-slate-400 font-normal text-xs ml-1">(único, sin espacios)</span>
        </label>
        <input
          type="text"
          bind:value={code}
      ...
      <div>
        ...
        <p class="mt-1 text-xs text-slate-400">Dejar en 0 si la caja comienza vacía.</p>
      </div>
