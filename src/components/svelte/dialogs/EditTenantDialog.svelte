<script lang="ts">
    ...
    import { Calendar, CircleAlert, CircleCheck, Pencil, X } from "@lucide/svelte";
    ...
    // Form fields — pre-filled from tenant prop
    let fullName = $state(tenant.fullName);
    let ci = $state(tenant.ci ?? "");
    let phone = $state(tenant.phone ?? "");
    let email = $state(tenant.email ?? "");
    let roomNumber = $state(tenant.roomNumber);
    let floor = $state(tenant.floor ?? "");
    let description = $state(tenant.description ?? "");
    let monthlyRent = $state(String(tenant.monthlyRent));
    let startDate = $state(tenant.startDate ? tenant.startDate.slice(0, 10) : "");
    let endDate = $state(tenant.endDate ? tenant.endDate.slice(0, 10) : "");
    let notes = $state(tenant.notes ?? "");
    ...
    async function handleSubmit(e: SubmitEvent) {
        ...
        isSubmitting = true;
        try {
            const result = await actions.inquilinos.updateTenant(fd);
...
>
    ...
    {#snippet children()}
        ...
        {:else}
            ...
            <form id="edit-tenant-form" onsubmit={handleSubmit} class="space-y-4 p-4">
                <!-- Nombre completo -->
                <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input
                            ...
                            placeholder="101"
                            class="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500
                            {fieldError('roomNumber') ? 'border-red-400' : 'border-slate-200'}"
                        />
                        {#if fieldError("roomNumber")}
                            <p class="mt-1 text-xs text-red-600">{fieldError("roomNumber")}</p>
                        {/if}
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input
                                ...
                                {fieldError('monthlyRent') ? 'border-red-400' : 'border-slate-200'}"
                            />
                        </div>
                        {#if fieldError("monthlyRent")}
                            <p class="mt-1 text-xs text-red-600">{fieldError("monthlyRent")}</p>
                        {/if}
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">
                        ...
                        <div class="relative">
                            <input
                                ...
                                {fieldError('startDate') ? 'border-red-400' : 'border-slate-200'}"
                            />
    ...
    {#snippet footer({ close })}
        {#if !successMsg}
            ...
            <button
                ...
                disabled={isSubmitting || !fullName || !roomNumber || !monthlyRent || !startDate}
