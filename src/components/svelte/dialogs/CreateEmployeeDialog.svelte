<script lang="ts">
  ...
  let { sectors, directorioCount, plantaCount, maxDirectorio, maxPlanta }: Props = $props();
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    if (!parts.name || !parts.pLastName) {
      inputErrors = { fullName: "Ingrese al menos nombre y primer apellido" };
      return;
    }
    ...
    fd.set("name", parts.name);
    fd.set("pLastName", parts.pLastName);
    if (parts.mLastName) fd.set("mLastName", parts.mLastName);
    ...
    try {
      ...
      if (result?.error) {
        serverError = result.error.message ?? "Error al registrar empleado.";
        return;
      }
...
<Dialog bind:isOpen size="lg" title="Registrar Empleado" description="Complete los datos del nuevo empleado" onClose={reset}>
  ...
  {:else}
    {#if serverError}
      <div class="mb-4 flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 px-4 py-3">
        ...
        <button onclick={() => (serverError = null)} class="text-red-400 hover:text-red-600">
          <X class="h-4 w-4" />
        </button>
      </div>
    {/if}
    ...
    <form onsubmit={handleSubmit} class="space-y-5">
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-3">
          <label class="relative flex cursor-pointer items-center gap-3 rounded-lg border-2 p-3 transition-colors
            {employeeType === 'directorio' ? 'border-blue-500 bg-blue-50' : 'border-slate-200 hover:border-blue-300'}">
            <input type="radio" bind:group={employeeType} value="directorio" class="sr-only" />
            <div class="flex-1">
              <p class="font-semibold text-slate-800 text-sm">Directorio</p>
              <p class="text-xs text-slate-500">{directorioCount}/{maxDirectorio} activos</p>
            </div>
          ...
          <label class="relative flex cursor-pointer items-center gap-3 rounded-lg border-2 p-3 transition-colors
            {employeeType === 'planta' ? 'border-violet-500 bg-violet-50' : 'border-slate-200 hover:border-violet-300'}">
            <input type="radio" bind:group={employeeType} value="planta" class="sr-only" />
            <div class="flex-1">
              <p class="font-semibold text-slate-800 text-sm">Planta</p>
              <p class="text-xs text-slate-500">{plantaCount}/{maxPlanta} activos</p>
            </div>
      ...
      <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">
          ...
          <span class="text-slate-400 font-normal text-xs ml-1">(Nombre Apellido1 Apellido2)</span>
        </label>
        <input
          type="text"
          bind:value={fullName}
          ...
          class="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500
            {fieldError('fullName') || fieldError('name') || fieldError('pLastName') ? 'border-red-400' : 'border-slate-200'}"
        />
        {#if fieldError('fullName')}
          <p class="mt-1 text-xs text-red-600 flex items-center gap-1"><CircleAlert class="h-3 w-3" />{fieldError('fullName')}</p>
        {:else if fieldError('name') || fieldError('pLastName')}
          <p class="mt-1 text-xs text-red-600 flex items-center gap-1"><CircleAlert class="h-3 w-3" />{fieldError('name') ?? fieldError('pLastName')}</p>
        {:else if fullName.trim().split(/\s+/).length >= 2}
          <p class="mt-1 text-xs text-emerald-600 flex items-center gap-1">
            <CircleCheck class="h-3 w-3" />
            {fullName.trim().split(/\s+/).slice(0,3).join(" · ")}
          </p>
        {/if}
      </div>
      ...
      <div>
        ...
        {#if fieldError('chargeTitle')}
          <p class="mt-1 text-xs text-red-600">{fieldError('chargeTitle')}</p>
        {/if}
      </div>
      ...
      <div>
        ...
        {#if fieldError('sectorId')}
          <p class="mt-1 text-xs text-red-600">{fieldError('sectorId')}</p>
        {/if}
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          {#if fieldError('hireDate')}
            <p class="mt-1 text-xs text-red-600">{fieldError('hireDate')}</p>
          {/if}
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">
          ...
          <div class="relative">
            ...
            <input
              ...
              class="w-full rounded-lg border pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500
                {fieldError('baseSalary') ? 'border-red-400' : 'border-slate-200'}"
            />
          </div>
          {#if fieldError('baseSalary')}
            <p class="mt-1 text-xs text-red-600">{fieldError('baseSalary')}</p>
          {/if}
        </div>
      </div>
      ...
      <div class="flex items-center justify-between border-t border-slate-100 pt-4">
        ...
        <button
          type="submit"
          disabled={isSubmitting || !fullName || !ci || !chargeTitle || !sectorId || !hireDate || !baseSalary}
