<script lang="ts">
    ...
    function openPrint() {
        window.open(
            `/print/recibo?id=${transactionId}&back=${printBack}`,
            "_blank",
        );
    }
</script>
...
<div class="rounded-xl border border-slate-200 overflow-hidden">
    ...
    <div class="{hdr} px-5 py-4 flex items-center gap-4">
        ...
        <div class="flex-1 min-w-0">
            ...
            {#if reference}
                <span
                    class="inline-block mt-1 font-mono text-xs {badge} px-2 py-0.5 rounded"
                >
                    {reference}
                </span>
            {/if}
    ...
    <div class="bg-slate-50 border-t border-slate-200 px-5 py-3 space-y-2">
        ...
        <button
            type="button"
            onclick={openPrint}
            class="w-full flex items-center justify-center gap-2 rounded-lg border bg-white px-4 py-2 text-sm font-medium transition-colors {pBtn}"
        >
            <svg
                class="h-4 w-4"
                fill="none"
            ...
            >
                <path
                    ...
                    d="M6 9V2h12v7M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2M9 21h6v-4H9v4z"
                />
            </svg>
