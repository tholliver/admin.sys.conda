<script lang="ts">
    ...
    import {
        Pencil, CircleAlert, CircleCheck,
        X, Trash2, TriangleAlert, Link2,
    } from "@lucide/svelte";
    ...
    let {
        ...
        allCashboxes = [],
        triggerLabel = "Editar",
        triggerClass = "inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-100 transition-colors",
    }: Props = $props();

    type InnerMode = "edit" | "deactivate-confirm" | "delete-confirm";

    let isOpen       = $state(false);
    let innerMode    = $state<InnerMode>("edit");
    ...
    let selectedCashboxId = $state(initialCashboxId);
    ...
    function reset() {
        ...
        selectedCashboxId = initialCashboxId;
        inputErrors       = {};
        serverError       = null;
        successMsg        = null;
        innerMode         = "edit";
    ...
    async function handleSubmit(e: SubmitEvent) {
        ...
        if (selectedCashboxId && selectedCashboxId !== initialCashboxId)
            fd.set("cashboxId", selectedCashboxId);
        ...
        try {
            ...
            if (result?.error) {
                serverError = result.error.message ?? "Error al actualizar el sector.";
                return;
            }
            if (result?.data?.success) {
                successMsg = result.data.message ?? "Sector actualizado.";
                setTimeout(() => { isOpen = false; reset(); window.location.reload(); }, 900);
            }
        } catch (err: any) {
...
<Dialog
    ...
    size="md"
    title="Editar Sector"
    description="Actualice la configuración del sector."
...
>
    ...
    {#snippet children()}
        ...
        {:else if innerMode === "deactivate-confirm"}
            <div class="space-y-4 py-2">
                <div class="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                    <p class="font-semibold mb-1">¿Desactivar "{initialName}"?</p>
                    <p class="text-xs text-amber-700">
                        El sector quedará inactivo. El personal asignado seguirá existiendo pero no
                        podrá vincularse a una caja ni generar cuotas. Esta acción puede revertirse
                        editando el sector directamente en la BD.
                    </p>
                </div>
                {#if serverError}
                ...
                <div class="flex gap-2 justify-end">
                    ...
                    <button
                        ...
                        class="rounded-lg bg-amber-500 px-4 py-2 text-sm font-semibold text-white hover:bg-amber-600 transition-colors disabled:opacity-50 inline-flex items-center gap-2"
                    >
                        {#if isSubmitting}
        ...
        {:else if innerMode === "delete-confirm"}
            <div class="space-y-4 py-2">
                <div class="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-800">
                    <p class="font-semibold mb-1">¿Eliminar definitivamente "{initialName}"?</p>
                    <p class="text-xs text-red-700">
                        Esta acción no se puede deshacer. Las transacciones históricas vinculadas quedarán
                        con referencia nula pero no se perderán.
                    </p>
                </div>
                {#if serverError}
                    <div class="flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 px-4 py-3">
    ...
    {#snippet footer({ close })}
        {#if !successMsg && innerMode === "edit"}
            <!-- Destructive action on the left -->
            {#if empCount === 0}
                <button
                    type="button"
                    onclick={() => (innerMode = "delete-confirm")}
            ...
            {:else}
                <button
                    type="button"
                    onclick={() => (innerMode = "deactivate-confirm")}
                    class="rounded-lg border border-amber-200 px-3 py-2 text-xs font-medium text-amber-700 hover:bg-amber-50 transition-colors sm:mr-auto inline-flex items-center gap-1.5"
                >
        ...
        {:else if successMsg || innerMode !== "edit"}
            <!-- Confirm sub-modes render their own buttons inside children -->
            {#if successMsg}
                <button
                    type="button"
                    onclick={close}
